...
tray_slot_depth = 2.5;
spacer_depth = 3;
rounding = 5; // Set to zero to avoid rounded corners
...
difference() {
    ...
    if (rounding > 0) {
        // punch a rounded hole for the sd card
        translate([ (outer_width - sd_window_width) / 2,
                    length - back_wall_thickness - epsilon,
                    tray_depth + floor_depth+sd_window_height]) {
            rotate([270,0,0]) {
                linear_extrude(back_wall_thickness + 2*epsilon) {
                    roundedsquare([sd_window_width, height - 2*tray_depth - 2*floor_depth], rounding);
                }
            }
        }
        // punch a rounded hole for side port access
        translate([ -epsilon,
                    pillar_width,
                    pcb_height]) {
            rotate([90,0,90]) {
                linear_extrude(outer_width + 2*epsilon) {
                    roundedsquare([port_window_width, height - pcb_height - tray_depth - floor_depth], rounding);
                }
            }
        }
    } else {
