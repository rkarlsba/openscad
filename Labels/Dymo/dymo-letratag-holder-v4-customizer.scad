 ...
 * 
 * Version 4.0 - 2025-08-05 by Roy Sigurd Karlsbakk <roy@karlsbakk.net>
 * - Whereever v2 and v3 went, dunno, but apparently, the file from the
 *   thingiverse and printables links above, is v3, so I guess this must be v4.
 * - Code cleanup.
 * - Removed commented out code.
 * - Changed $fn from 250 to 128 to speed things up a bit - you won't see the
 *   visual difference.
 ...
 * - Added shell script to export downloadable STL files.
 *
 * FIXME:
...
module upper_part() {
    ...
    if (hinges) {
        ...
        segs = floor(hinge_width/hinge_seg);
        _segs = (segs % 2) ? segs : segs - 1;
        translate([b_x+mat*2-1,case_width,b_z_top-hinge_height+2]) {
            mirror([1,0,0])
            cuboid([mat/2,case_width,hinge_height], anchor=BOTTOM+LEFT+BACK) {
                position(TOP+LEFT) {
                    orient(anchor=LEFT) {
