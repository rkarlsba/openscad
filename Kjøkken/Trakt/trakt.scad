  ...
  translate([0,0,-Height]) {
    ...
    translate([0,0,Height-Wall_Thickness*2]) {
      difference() {
        ...
        translate([0,0,-0.1]) {
          cylinder(d = Top_Inside_Diameter, h = Wall_Thickness*2+0.2);
        }
      }
    }
    
    // Funnel top ring lip
    translate([(Top_Outside_Diameter+Ledge*3)/2,0,Height-Wall_Thickness*2]) {
      difference() {
        union() {
          translate([-Ledge*1.5-Wall_Thickness,-Ledge*Ledge_Multiplicator/2,0]) {
            cube([Ledge*1.5+Wall_Thickness,Ledge*Ledge_Multiplicator,Wall_Thickness*2]);
          }
          cylinder(d = Ledge*Ledge_Multiplicator, h = Wall_Thickness*2);
        }
        translate([0,0,-0.1]) {
          cylinder(d = Ledge*Ledge_Multiplicator-2*Ledge, h = (Wall_Thickness*2)+0.2);
        }
      }
    }
    ...
    translate([0,0,Tube_Height]) {
      difference() {
        union() {
          ...
          translate([0,Tube_Outside_Diameter/2,0]) {
            rotate([-atan((Top_Outside_Diameter/2 - Tube_Outside_Diameter/2)/(Top_Height-Wall_Thickness)),0,0]) {
              union() {
                cylinder(d = Wall_Thickness, h = Top_Height-Wall_Thickness);
                translate([0,0,Top_Height-Wall_Thickness]) {
        ...
        translate([0,0,+0.1]) {
          cylinder(d1 = Tube_Outside_Diameter - Wall_Thickness*2, d2 = Top_Outside_Diameter - Wall_Thickness*2+0.2, h = Top_Height-Wall_Thickness);
        }
        translate([0,0,-0.1]) {
          cylinder(d = Tube_Outside_Diameter - Wall_Thickness*2, h = 0.3);
        }
      }
    }
    ...
    difference() {
      union() {
        ...
        translate([0,Tube_Outside_Diameter/2,0]) {
          cylinder(d = Wall_Thickness, h = Tube_Height);
        }
      }
      translate([0,0,-0.1]) {
        cylinder(d = Tube_Outside_Diameter - Wall_Thickness*2, h = Tube_Height+0.2);
      }
      translate([-Tube_Outside_Diameter/2,-Tube_Outside_Diameter/2,0]) {
        rotate([Tube_Cut_Angle,0,0]) {
          mirror([0,0,1]) {
            cube([Tube_Outside_Diameter,Tube_Height*3,Tube_Outside_Diameter]);
          }
        }
      }
    }
