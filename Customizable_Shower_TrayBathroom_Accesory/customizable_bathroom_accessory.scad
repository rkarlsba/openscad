    ...
    translate([0,r1+0.2,0]) {
        union() {
            difference() {
                ...
                translate([-r1*2,r1*0.6,-1])
                    cube([r1*4,r1*2,h1+2]);
            }
            rotate([0,0,35])
                translate([r1+1.25,0,0])
                    cylinder(h2,r2,r2);
            rotate([0,0,145])
                translate([r1+1.25,0,0])
                    cylinder(h2,r2,r2);
...
module roundedrectangle(width,length,height,r1) {
    module cross() {
        union() {
            translate([0,r1,0])
                cube([width,length-(r1*2),height]);
            translate([r1,0,0])
                cube([width-(r1*2),length,height]);
        }
    }
    ...
    union() {
        ...
        translate([width-r1,r1,0])
            cylinder(height,r1,r1);
        translate([r1,length-r1,0])
            cylinder(height,r1,r1);
        translate([width-r1,length-r1,0])
            cylinder(height,r1,r1);
    }
}
...
module tray(width,length,height) {
    difference() {
        if (corner==2) {  
            translate([-(width+wall_thickness*2)/2,-(length+wall_thickness*2),0])
                cube([width+wall_thickness*2,length+wall_thickness*2,height]);
        } else { 
            translate([-(width+wall_thickness*2)/2,-(length+wall_thickness*2),0])
                roundedrectangle(width+wall_thickness*2,length+wall_thickness*2,height,1.75);
        }
        translate([-width/2,-length-wall_thickness,-1])
...
module pattern() {
    translate([0,0,-2.4]) {
        difference() {
            union() {
                intersection() {
                    translate([-(w+wall_thickness)/2,-(l+wall_thickness+0.5*wall_thickness),0])
                        cube([w+wall_thickness,l+wall_thickness,4.8]);
                    scale([1,1,10])
                        honeycomb(pattern_width,pattern_length,pattern_height,honeycomb_radius,honeycomb_radius_modifier,honeycomb_thickness,honeycomb_sides);
                }
            }
            translate([-(w+wall_thickness+0.1)/2,-(l+wall_thickness+0.5*wall_thickness)-0.05,-0.1])
                cube([w+wall_thickness+0.1,l+wall_thickness+0.1,2.5]);
        }
    }
}
