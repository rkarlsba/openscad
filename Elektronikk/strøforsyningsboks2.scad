...
module bunn() {
    difference() {
        ...
        if (avrunda_spalter) {
            ...
            for (fy=[8:7:y-3]) {
                hull() {
                    translate([10+thickness,fy,-renderfix]) {
                        cylinder(d=slitwidth, h=thickness+renderfix*2);
                    }
                    translate([x-10+thickness,fy,-renderfix]) {
            ...
            for (fy=[8:7:y-3]) {
                translate([10,fy,-renderfix]) {
                    cube([x-20+thickness*2,slitwidth,thickness+renderfix*2]);
                }
            }
        }
        if (lag_usbhull) {
        ...
        for (fz=[8:7:z]) {
            for (fx=[10:10:x-5]) {
                translate([fx,-renderfix,fz]) {
                    rotate([270,0,0]) {
                        cylinder(r=holesize, h=y+thickness*2+renderfix*2);
                    }
                }
            }
        }
        for (fz=[8:7:z]) {
            for (fy=[10.5:10:y-5]) {
                translate([x+thickness*2+renderfix,fy,fz]) {
                    rotate([0,270,0]) {
                        cylinder(r=holesize, h=thickness+renderfix*2);
                    }
                }
            }
        }
