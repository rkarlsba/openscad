...
psu_z_gap = 10;
wall = 3;
x_outer = psu[0]+psu_x_gap*2+wall*2;
z_outer = psu[2]+psu_z_gap*2+wall*2;
x_inner = psu[0]+psu_x_gap*2;
...
module inner_frame(x=0, tolerance=tolerance, depth=depth) {
    ...
    translate([wall,0,0]) {
        rotate([0,270,0]) {
            color("red") plate([depth,z_inner*tolerance_num], wall, 3);
        }
    }
    translate([0,(z_inner+wall)*tolerance_num,0]) {
        rotate([90,0,0]) {
            color("yellow") plate([(x_inner)*tolerance_num,depth], wall, 3);
        }
    }
    translate([x_inner*tolerance_num+wall,wall,0]) {
        rotate([0,270,0]) {
            color("lightblue") plate([depth,z_inner*tolerance_num], wall, 3);
        }
    }
    translate([wall,wall,0]) {
        rotate([90,0,0]) {
            color("pink") plate([(x_inner)*tolerance_num,depth], wall, 3);
        }
    }
}
    ...
    translate([wall,wall,0]) {
        difference() {
            linear_extrude(wall) {
                difference() {
                    square([x_inner,z_inner]);
                    translate([25-2,(z_inner-term_clips_h)/2]) {
                        square([term_x+4,term_clips_h]);
                    }
                    translate([25,(z_inner-term_y)/2]) {
                        square([term_x,term_y]);
                    }
                    translate([9,z_inner/3]) {
                        circle(d=4.2, $fn=fn*2);
                    }
                    translate([9,z_inner/3*2]) {
                        circle(d=4.2, $fn=fn*2);
                    }
                }
            }
...
//inner_frame();
translate([psu_x_gap+wall,front[1]+psu_front_gap,wall]) 
{
    //rotate([90,0,0]) 
    {
        draw_psu();
