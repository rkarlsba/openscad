...
d4s_num_4_h_push=-3;
d4s_body_length=1.0;
d4s_point_length=0.5;
d4s_text=[""," ",""," ","",""];
d4s_symbols=[undef,undef,"|||",undef,undef,"| |"];
d4s_symbol_size=72;
d4s_symbol_v_push=0;
d4s_symbol_h_push=0;
d4s_rotate=[0,0,90,0,0,90];
d4s_pips=[" "," "," "," "," "," "];
d4s_pip_sides=6; //[0,3,4,5,6,8,10,12]
d4s_pip_size=20;
d4s_pip_offset=2.5;
d4s_pip_symbol_pos=["1"," ","3"," ","4","2"];
d4s_pip_symbols=[undef,undef,undef,undef,undef,undef]; //symbols for pips
d4s_pip_symbol_rotate=[0,0,0,0,0,0];
d4s_adj_size=[0,0,0,0,0,0];
d4s_adj_v_push=[0,0,0,0,0,0];
d4s_adj_h_push=[0,0,0,0,0,0];
d4s_adj_depth=[0,0,0,0,0,0];
...
if(d4s) move([-spacing,-spacing]) drawd4s();
...
module drawd4s(){
    txt_merged=merge_txt(d4s_text,fix_quotes(d4s_symbols));
    txt_mult=d4s_text_size*d4s_size/100;
    adj_txt=adj_list(d4s_adj_size,d4s_size/100);
    txt_stroke=text_stroke*txt_mult;
    sym_mult=d4s_symbol_size*d4s_size/100;
    sym_stroke=symbol_stroke*sym_mult;
    base_rotate=[180,0,90,0,0,-90];
    d4s_pip_fn=d4s_pip_sides==0 ? 128 : d4s_pip_sides;
    d4s_body_length=d4s_body_length*d4s_size;
    d4s_point_length=d4s_point_length*d4s_size;
    circumsphere_dia=d4s_body_length+d4s_point_length*2;
    corner_round_mult=circumsphere_dia-(corner_rounding*circumsphere_dia/100)/1.9;
    corner_clip_mult=circumsphere_dia-(corner_clipping*circumsphere_dia/100)/1.9;
    ...
    difference()
    {
        intersection()
        {
            ...
            minkowski()
            {
                scale((d4s_size-2*edge_rounding)/d4s_size)
                {
                    cuboid([d4s_size,d4s_body_length,d4s_size],anchor=BOTTOM);
                    translate([0,-d4s_body_length/2,d4s_size/2])
                    rotate([90,90,0])
                    prismoid([d4s_size,d4s_size],[0,0],h=d4s_point_length);
                    mirror([1,0,0])
                    translate([0,d4s_body_length/2,d4s_size/2])
                    rotate([-90,-90,0])
                    prismoid([d4s_size,d4s_size],[0,0],h=d4s_point_length);
        ...
        //render numbers & symbols
        regular_polyhedron("cube",side=d4s_size,anchor=BOTTOM,draw=false)
        zrot(d4s_rotate[$faceindex]+base_rotate[$faceindex])
        down(text_depth+d4s_adj_depth[$faceindex])
        linear_extrude(height=2*text_depth+d4s_adj_depth[$faceindex])
        move([(d4s_text_h_push+d4s_adj_h_push[$faceindex])*d4s_size/100,(d4s_text_v_push+d4s_adj_v_push[$faceindex])*d4s_size/100])
        if(is_list(txt_merged[$faceindex])) //a symbol
            move([d4s_symbol_h_push*d4s_size/100,d4s_symbol_v_push*d4s_size/100])
            offset(delta=sym_stroke)
            text(txt_merged[$faceindex][0],size=sym_mult,font=sym_font,halign="center",valign="center");
        else if(txt_merged[$faceindex]=="4") //a number 4
            right(d4s_num_4_h_push*d4s_size/100)
            offset(delta=txt_stroke)
            text(txt_merged[$faceindex],size=txt_mult+adj_txt[$faceindex],font=txt_font,halign="center",valign="center");
        else //a number thats's not 4
            offset(delta=txt_stroke)
        ...
        regular_polyhedron("cube",side=d4s_size,anchor=BOTTOM,draw=false)
        zrot(d4s_rotate[$faceindex]+base_rotate[$faceindex])
        drwapips("d4s",d4s_pips[$faceindex],d4s_adj_depth[$faceindex],d4s_pip_fn);
        
        //render pip symbols
        d4s_pip_symbols=fix_quotes(d4s_pip_symbols);
        regular_polyhedron("cube",side=d4s_size,anchor=BOTTOM,draw=false)
        zrot(d4s_rotate[$faceindex]+base_rotate[$faceindex])
        drwapipsymbols("d4s",d4s_pip_symbol_pos[$faceindex],d4s_pip_symbols[$faceindex],d4s_pip_symbol_rotate[$faceindex],d4s_adj_depth[$faceindex]);
    }
}
